                                    ...
                                    size_t pms_offset )
{
    ...
    unsigned char tmp_mac[6] = {0xff, 0xff, 0xff, 0x00, 0x00, 0x00};
#endif
    int ret;
...
#ifdef CLOUD_AUTH
    pmac = ssl->handshake->premaster;
    pmac = pmac + ssl->handshake->pmslen - 6;
    lega_wlan_get_mac_address(tmp_mac);
    memcpy(pmac, tmp_mac, 6);
    pmac = NULL;
#endif
